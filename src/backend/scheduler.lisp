       ...
       do
         (cond ((eq (ir::opcode inst) :|mult|)
               ...
               (t (if (or (null reg1)
                      ...
                      (when (null reg2)
                        (setf reg2 i)
                        (setf reg2-prio (node-priority node)))))))
    ;; (format t "mul: ~a, ~a~%mem: ~a, ~a~%reg1: ~a, ~a~%reg2: ~a, ~a~%"
    ;;         mul-inst mul-prio mem-inst mem-prio reg1 reg1-prio reg2 reg2-prio)
  ...
  (let ((ll (ll::make-LL)))
    ...
    (let ((active '())
      ...
      (loop while (> left 0)
         do
           ;; (format t "~a~%" *ready*)
           ;; (mapcar (lambda (x) (format t "~a," (ir::string-instruction (node-inst (aref *node-table* x)) #'ir::virtual))) *ready*)
           ;; (format t "~%")
