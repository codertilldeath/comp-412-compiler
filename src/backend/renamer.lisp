...
(defun update (ir count)
  ...
  (let ((s (ir::source (ir::r2 ir))))
    (unless (= s -1)
      (when (= (aref *SR-to-VR* s) -1)
        (setf (aref *SR-to-VR* s) *VR-name*)
        (incf *VR-name*))
      (setf (ir::virtual (ir::r2 ir)) (aref *SR-to-VR* s))
      (setf (ir::next-use (ir::r2 ir)) (aref *last-use* s))
      (setf (aref *last-use* s) count)))
  
  (let ((s (ir::source (ir::r1 ir))))
    (unless (= s -1)
      (when (= (aref *SR-to-VR* s) -1)
        (setf (aref *SR-to-VR* s) *VR-name*)
        (incf *VR-name*))
      (setf (ir::virtual (ir::r1 ir)) (aref *SR-to-VR* s))
      (setf (ir::next-use (ir::r1 ir)) (aref *last-use* s))
      (setf (aref *last-use* s) count))))
