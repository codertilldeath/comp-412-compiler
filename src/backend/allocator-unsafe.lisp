...
(defun choose-spill-register (dont-use linum)
  (let (max-next-use max-register
    ...
    (cond ((null max-remat) (or max-clean max-register))
          ((and (null max-register)
                (null max-clean))
           max-remat)
          ((and (not (null max-clean))
                (aref *high-pressure-zones* max-nu-remat)
                (not (aref *high-pressure-zones* max-nu-clean)))
           max-clean)
          ((and (not (null max-register))
                (aref *high-pressure-zones* max-nu-remat)
